...
import { 
  ...
  type InvestmentType, type InsertInvestmentType, 
...
export class MongoStorage implements IStorage {
  ...
  constructor() {
    ...
    this.investmentTypes = this.db.collection<InvestmentType>('investmentTypes');
  ...
  private async initializeDefaultTypes(): Promise<void> {
    try {
      ...
      const existingTypes = await this.investmentTypes.countDocuments({ userId: defaultUserId, isDefault: true });
      const existingCategories = await this.billCategories.countDocuments({ userId: defaultUserId, isDefault: true });
  ...
  async getAllTransactionsForUser(userId: string): Promise<Transaction[]> {
    ...
    return await this.transactions.find({ investmentId: { $in: investmentIds } }).toArray();
  }
  ...
  async updateBillPayment(id: string, updateData: Partial<InsertBillPayment>): Promise<BillPayment | undefined> {
    ...
    const result = await this.billPayments.findOneAndUpdate(
      ...
      { 
        $set: { 
          ...updateData,
          amount: updateData.amount !== undefined ? updateData.amount.toString() : oldPayment.amount
        } 
      },
      { returnDocument: 'after', includeResultMetadata: false }
    );
    ...
    if (oldStatus === "paid" && updateData.status && updateData.status !== "paid") {
      ...
      await this.updateBillAfterPayment(oldPayment.billId, oldPayment.paidDate, oldPayment.dueDate);
    }
